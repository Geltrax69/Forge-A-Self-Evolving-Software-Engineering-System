...
Go has two allocation primitives, the built-in functions `new` and `make`. They do different things and apply to different types, which can be confusing, but the rules are simple. Let's talk about `new` first. It's a built-in function that allocates memory, but unlike its namesakes in some other languages it does not _initialize_ the memory, it only _zeros_ it. That is, `new(T)` allocates zeroed storage for a new variable of type `T` and returns its address, a value of type `*T`. In Go terminology, it returns a pointer to a newly allocated zero value of type `T`.
...
With our recovery pattern in place, the `do` function (and anything it calls) can get out of any bad situation cleanly by calling `panic`. We can use that idea to simplify error handling in complex software. Let's look at an idealized version of a `regexp` package, which reports parsing errors by calling `panic` with a local error type. Here's the definition of `Error`, an error type for the regexp package, a `Parse` function that panics when an error is detected, and a `Compile` function that uses `recover` to set the error return value:
...
func Compile(str string) (*Regexp, error) {
    regexp := &Regexp{expr: str}
    safelyParse(regexp)
    return regexp, regexp.err
}

func safelyParse(regexp *Regexp) {
    defer func() {
        if e := recover(); e != nil {
            regexp.err = e.(Error)
        }
    }()
    regexp.prog = parse(regexp.expr)
}
```

If `parse` panics, the recovery block will set `regexp.err` to the error value, and the program will resume from `safelyParse` without crashing. It doesn't strictly need a named type for the error value, but it's nice for the `panic/recover` mechanism to separate package errors from genuine execution panics.

With this package-level error handling in place, the `parse` function (and any other code it calls) can report a parse problem cleanly by calling `panic`. This simplifies error handling inside the parse logic.

Note that this recovery pattern only works within the same package. The `parse` function must call `panic` with a value of the local error type; it won't work if `parse` calls `panic` with some other value. For this reason—and because `recover` always returns `nil` unless called from a deferred function—it's good practice for packages to provide only exported functions that call `panic`, never exported functions that `panic` themselves.

## A web server

Let's finish with a complete, working example: a web server. This is actually a kind of re-implementation of part of the `net/http` package, but simpler.
...
// Handler is called by the server.
type Handler interface {
    ServeHTTP(c *Conn, req *Request)
}

// The HandlerFunc type is an adapter to allow the use of
// ordinary functions as HTTP handlers.  If f is a function
// with the appropriate signature, HandlerFunc(f) is a Handler
// object that calls f.
...
// Argument server.
func ArgServer(c *Conn, req *Request) {
    fmt.Fprint(c, "Argument is: ", req.FormValue("ignore_me"))
}

// Simple counter server. The arguments to ServeHTTP are currently
// the connection and the parsed URL. In a finished
// http server, the Conn and Request will be fully populated
// with cookies and other HTTP state. For now we'll keep it simple.
type Counter struct {
    n int
}
...
// Argument server, variation 2. Returns in HTML and picks out
// query arguments neatly. Suppose you run with
//   gorun webserver.go
// and visit yourself on port 6060.
//
// /Arg?name=egreg prints
// <h1>Egreg</h1>I said <i>egreg</i>.
//
// /Nobucket/play.html prints
// didn't find /Nobucket/play.html.
//
// /arg?stuff=emptiness prints
// stuff too short.
type ArgServer string

// Homemade server, variation 2.
func (s ArgServer) String() string {
    return string(s)
}

func (s ArgServer) ServeHTTP(c *Conn, req *Request) {
    clientip := c.RemoteAddr()
    fmt.Fprintf(c, "Client IP:%s", clientip)
    switch req.FormValue("ignore_me") {
    case "":
        fmt.Fprint(c, "stuff too short.")
    case "123":
        fmt.Fprintf(c, "item <%s>: ", req.FormValue("item"))
        if req.FormValue("item") == "" {
            fmt.Fprint(c, "item cannot be empty.")
        }
    default:
        fmt.Fprintf(c, "item <%s>", req.FormValue("ignore_me"))
    }
}
```

Notice that we've made a complete working web server with only a few tens of lines of code, thanks to Go and its libraries. You can make a web server into a standalone executable by including an `http.ListenAndServe` call in an `init` function. After the server starts, it will handle requests on port 6060 on any machine that runs it:
...
(The `main` function is empty; without one, Go executes the init functions and then continues to run the binary indefinitely.)

For details about how the `http` packages work, see [the documentation](/pkg/net/http).
----
