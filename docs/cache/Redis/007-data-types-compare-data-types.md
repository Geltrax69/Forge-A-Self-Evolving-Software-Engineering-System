...
Arrays store values addressed by integer index, making random access O(1) regardless of the array's logical size. Because arrays are sparse, setting an element at index 1,000,000 does not allocate memory for the million empty slots in between, so large index gaps are inexpensive. Arrays distinguish between *logical length* (the highest set index plus one, returned by [`ARLEN`]()) and *element count* (the number of non-empty slots, returned by [`ARCOUNT`]()).

In addition to direct index access, arrays support sequential insertion via [`ARINSERT`](), which advances an internal cursor automatically. The cursor can be repositioned with [`ARSEEK`](), enabling flexible append patterns. [`ARRING`]() makes the ring buffer pattern explicit: it inserts values modulo a fixed size, wrapping around and overwriting the oldest entries when the buffer is full. [`AROP`]() performs single-pass aggregations (sum, min, max, bitwise operations, value matching) over a range without fetching each element individually.
