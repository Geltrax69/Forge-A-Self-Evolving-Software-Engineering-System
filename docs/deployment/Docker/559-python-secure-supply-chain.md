...
Complete [Configure CI/CD for your Python application](https://docs.docker.com/guides/python/configure-github-actions/).
...
The Dockerfile uses two tags, `dhi.io/python:3.12-dev` in the builder stage and `dhi.io/python:3.12` in the runtime stage. Each tag has its own digest, so look up both:
...
Each digest is a 64-character hex string. Update your `Dockerfile` to reference each digest on the matching `FROM` line:
