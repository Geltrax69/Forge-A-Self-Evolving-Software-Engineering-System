...
Docker Content Trust (DCT) and the Notary v1 service at `notary.docker.io` are being fully retired. If you've never set `DOCKER_CONTENT_TRUST=1` or used `docker trust` commands, this change doesn't affect you. The service shuts down completely on December 8, 2026, following brownout windows in July and August. For the timeline, migration guidance, and modern alternatives such as Sigstore/Cosign and Notation, see the [Docker Content Trust retirement and migration blog post](https://www.docker.com/blog/docker-content-trust-retirement-and-migration-guidance/).
