...
## [Use Socket Firewall variants to monitor package installations](#use-socket-firewall-variants-to-monitor-package-installations)

If you want supply chain protection during dependency installation, use a Socket Firewall variant in place of the standard `-dev` variant in your build stage. These variants come with [Socket](https://socket.dev/) preinstalled to monitor package manager activity and block malicious packages before they reach your image.

Two tiers are available. Use `-sfw-dev` for Socket Firewall Free, or `-sfw-ent-dev` for Socket Firewall Enterprise (requires an API key from Socket). The runtime stage stays the same regardless of which build stage variant you use.
...
For more information on Socket Firewall variants, see [Available image types](https://docs.docker.com/dhi/explore/available/).
