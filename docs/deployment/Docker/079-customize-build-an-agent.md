...
For reference on every field, see the [Kit spec reference](https://docs.docker.com/ai/sandboxes/customize/kit-reference/). This tutorial focuses on the journey.
...
An agent kit needs a container image that satisfies the [base image requirements](https://docs.docker.com/ai/sandboxes/customize/kit-reference/#base-image-requirements): non-root `agent` user at UID 1000, passwordless sudo, `/home/agent/` home, and HTTP proxy environment variable forwarding.
...
## [Write the sandbox block](#write-the-sandbox-block)
...
* `aiFilename: AGENTS.md` tells the sandbox to create `AGENTS.md` at launch and append the [`agentContext`](#prime-amp-with-memory) block to it. Amp reads this file for instructions.
...
The `agentContext` field appends markdown to `AGENTS.md` at sandbox creation. Use it to tell Amp about the sandbox environment so it knows the conventions when it starts.
...
> Note
>
> `sbx secret set-custom` is experimental and may change in future releases. This tutorial surfaces it because there's no other path to register a custom-format placeholder.
...
Flesh out the `agentContext` block as you refine how Amp should behave in the sandbox.
...
The rest — agent-context block, network-policy iteration, packaging — is the same regardless of agent.
