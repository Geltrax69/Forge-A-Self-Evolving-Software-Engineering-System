...
[Explore VEX statements in Docker Hardened Images](https://docs.docker.com/guides/dhi-vex-walkthrough/)

Scan a Docker Hardened Image with and without VEX and audit every suppression and its justification.
...
* [Docker Desktop](https://www.docker.com/products/docker-desktop/), authenticated to `dhi.io`. Sign in with `docker login dhi.io`. Docker Desktop includes Docker Scout, which fetches the VEX attestation.
* A vulnerability scanner. This guide shows examples for Docker Scout, [Trivy](https://trivy.dev/), and [Grype](https://github.com/anchore/grype). Trivy and Grype can also run as Docker containers with no installation needed.
* `jq` (optional), for filtering the VEX file.

## [Expose daemon for Windows users](#expose-daemon-for-windows-users)
...
## [Step 1: Scan without VEX](#step-1-scan-without-vex)
...
## [Step 2: Fetch the VEX attestation](#step-2-fetch-the-vex-attestation)
...
## [Step 3: Scan with VEX applied](#step-3-scan-with-vex-applied)
...
## [Step 4: Inspect every suppression and its justification](#step-4-inspect-every-suppression-and-its-justification)
...
For the full list of justification codes, see [VEX status reference](https://docs.docker.com/dhi/core-concepts/vex/#not_affected-justification-codes).

Every suppression is documented, auditable, and verifiable with any VEX-enabled scanner.

## [Step 5: Read Docker's reasoning for a specific CVE](#step-5-read-dockers-reasoning-for-a-specific-cve)
...
## [Step 6: Filter VEX statements by status](#step-6-filter-vex-statements-by-status)
...
For status definitions and justification codes, see the [VEX status reference](https://docs.docker.com/dhi/core-concepts/vex/#vex-status-reference).

## [What's next](#whats-next)

* Scan with other tools: Learn how to apply DHI VEX statements with Trivy (VEX Hub) and Grype in [Scan Docker Hardened Images](https://docs.docker.com/dhi/how-to/scan/).
* Write your own VEX for child images: If you build on top of a DHI and want to suppress CVEs in packages you add, see [Create an exception using VEX](https://docs.docker.com/scout/how-tos/create-exceptions-vex/).
* VEX status reference: For status definitions, justification codes, and why DHI does not use `fixed`, see [Vulnerability Exploitability eXchange (VEX)](https://docs.docker.com/dhi/core-concepts/vex/#vex-status-reference).
* Browse the VEX feed directly: The raw VEX data is published at [github.com/docker-hardened-images/advisories](https://github.com/docker-hardened-images/advisories/tree/main/vex), organized by image name.
